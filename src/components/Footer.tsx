...
export default function Footer() {
  return (
    <footer className="bg-[var(--cream)] border-t border-[var(--gold-light)] py-12">
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-6">
          ...
          <div className="flex gap-8 text-xs tracking-[0.2em] uppercase">
            {[
              ...
              <a key={s.label} href={s.href} target="_blank" rel="noopener noreferrer" className="text-[var(--muted)] hover:text-[var(--gold)] transition-colors">
                {s.label}
              </a>
            ))}
          </div>
        </div>
        <div className="mt-8 pt-8 border-t border-[var(--gold-light)] flex flex-col md:flex-row items-center justify-between gap-4 text-xs text-[var(--muted)]">
